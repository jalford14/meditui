...
pub struct Plan {
    days: std::collections::HashMap<u16, Vec<ChapterRef>>,
}
...
impl Plan {
    pub fn new() -> Plan {
        ...
        for (i, line) in PLAN_DATA.lines().enumerate() {
            let day = (i + 1) as u16;
            ...
            days.insert(day, chapters);
        }

        Plan { days }
    }

    pub fn chapters_for_day(&self, day: u16) -> Vec<ChapterRef> {
        self.days.get(&day).cloned().unwrap_or_default()
    }
}

/// Map book names from McHeyne data to match KJV JSON book names
...
fn parse_reference(reference: &str) -> Vec<ChapterRef> {
    ...
    // Handle comma-separated chapters: "36,45"
    if spec.contains(',') {
        return spec
            .split(',')
    ...
    // Handle verse references (contains colon)
    if spec.contains(':') {
        return parse_verse_reference(&book, &spec);
    }

    // Handle simple chapter range: "9-10"
    if spec.contains('-') {
...
fn split_book_and_spec(reference: &str) -> (String, String) {
    // Find the last space before a character that starts with a digit
...
// Format: Family1|Family2|Secret1|Secret2
const PLAN_DATA: &str = "\
